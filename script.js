document.addEventListener("DOMContentLoaded", function () {
  ...
  window.startTimer = function () {
    ...
    document.getElementById("startClockDisplay").innerText =
      "Start Time: " + startClock;
  ...
  function restoreInputs() {
    ...
    document.getElementById("studentLandings").innerText =
      `Student Landings: ${localStorage.getItem("studentLandings") || 0}`;
    document.getElementById("instructorLandings").innerText =
      `Instructor Landings: ${localStorage.getItem("instructorLandings") || 0}`;
    document.getElementById("startClockDisplay").innerText =
      "Start Time: " + startClock;
  }
  ...
  window.calculateHobbs = function () {
    let startVal = document
      .getElementById("hobbsStart")
      .value.replace(/[^0-9.]/g, "");
    let endVal = document
      .getElementById("hobbsEnd")
      .value.replace(/[^0-9.]/g, "");
    let start = parseFloat(startVal) || 0;
    let end = parseFloat(endVal) || 0;
    let res = Math.floor((end - start) * 100) / 100;
    document.getElementById("hobbsResult").innerText =
      `Hobbs Time: ${res.toFixed(2)} hrs`;
  };

  window.calculateTach = function () {
    let startVal = document
      .getElementById("tachStart")
      .value.replace(/[^0-9.]/g, "");
    let endVal = document
      .getElementById("tachEnd")
      .value.replace(/[^0-9.]/g, "");
    let start = parseFloat(startVal) || 0;
    let end = parseFloat(endVal) || 0;
    let res = Math.floor((end - start) * 100) / 100;
    document.getElementById("tachResult").innerText =
      `Tach Time: ${res.toFixed(2)} hrs`;
  };
  ...
  window.calculateElapsedTime = function () {
    ...
    document.getElementById("elapsedResult").innerText =
      `Elapsed Time: ${floored.toFixed(2)} hrs`;
  };
  ...
  window.addLanding = function (type) {
    ...
    document.getElementById(key).innerText =
      `${type.charAt(0).toUpperCase() + type.slice(1)} Landings: ${val}`;
  };
